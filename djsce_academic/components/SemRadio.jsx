...
const SemRadio = ({ name, selectedOption, setSelectedOption }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-1 grid-rows-4 w-full text-amber-800 text-xl font-mono font-bold place-items-center">
      {sems.map((sem) => (
        <div key={sem} className="flex cursor-pointer gap-1 px-6 text-xl justify-self-start">
          <input
