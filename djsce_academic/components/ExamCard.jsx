...
const ExamCard = (props) => {
  ...
  return (
    <div className="p-4 flex flex-col gap-2 items-center border bg-[#0f0f0f] border-[#2a0a0a] rounded-2xl">
      {/* Title scales from text-2xl up to text-4xl */}
      <p className="font-bold text-2xl sm:text-3xl lg:text-4xl text-[#e8e0d8]">{title}</p>
      {/* Subtitle scales from text-sm up to text-xl */}
      <p className="text-sm sm:text-base lg:text-xl text-[#cc2222] mb-3">{subtitle}</p>
      {/* Button text scales from text-xs up to text-base */}
      <button
        className="flex gap-3 items-center justify-center hover:scale-105 cursor-pointer bg-[#aa1111] border-2 border-[#cc2222] font-mono hover:bg-[#1a0505] hover:text-[#ff4444] rounded-3xl w-full h-10 text-white text-xs sm:text-sm lg:text-base mt-auto"
        onClick={() => {
